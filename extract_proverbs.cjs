...
function generateExcelFile(proverbs, filename, includeCodeSheet = true) {
  ...
  const outputPath = generateVersionedFilename(filename);
...
function processRomanianProverbs(inputText, sourceFilename = "input.pdf") {
  ...
  const outputFilename = generateExcelFile(
    processedProverbs,
...
if (require.main === module) {
  try {
    const result = processRomanianProverbs(romanianText, "Enciclopedia înţelepciunii 7-17.pdf");
    console.log("\n" + "=".repeat(50));
